...
/* [Wataru Inariba] Some features are copyed from avisynth 2.6 */
struct VideoInfo {
...
//bool IsYUV9()  const { return (pixel_type & CS_PLANAR_MASK) == (CS_YUV9  & CS_PLANAR_FILTER); }
  bool IsY8()    const { return (pixel_type & CS_PLANAR_MASK) == (CS_Y8    & CS_PLANAR_FILTER); }
  bool IsColorSpace(int c_space) const { return IsPlanar() ? ((pixel_type & CS_PLANAR_MASK) == (c_space & CS_PLANAR_FILTER)) : ((pixel_type & c_space) == c_space); }
  ...
  bool IsVPlaneFirst() const { return !IsY8() && IsPlanar() && (pixel_type & (CS_VPlaneFirst | CS_UPlaneFirst)) == CS_VPlaneFirst; }  // Don't use this
  int BytesFromPixels(int pixels) const { return !IsY8() && IsPlanar() ? pixels << ((pixel_type>>CS_Shift_Sample_Bits) & 3) : pixels * (BitsPerPixel()>>3); }   // Will not work on planar images, but will return only luma planes
...
//        return 32;
  ...
  /* [Wataru Inariba] Added for convenience */
  int GetRowSize(int plane=0) const { return RowSize(plane); }
  int GetHeight(int plane=0) const {
    if (plane == PLANAR_U || plane == PLANAR_V)
      return (!IsY8() && IsPlanar()) ? height>>GetPlaneHeightSubsampling(plane) : 0;
    return height;
  }
};
...
public:
  ...
  /* [Wataru Inariba] These functions are not compatible with avisynth 2.6. Use the same functions in VideoInfo instead. */
  /*
